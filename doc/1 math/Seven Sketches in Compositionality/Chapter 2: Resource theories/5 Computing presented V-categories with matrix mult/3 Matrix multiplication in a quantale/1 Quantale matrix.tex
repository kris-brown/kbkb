% TAG Def
A \emph{matrix with entries in} $\mathcal{V}$, where $\mathcal{V}=(V, \leq, \otimes, I)$ is a \hyperref[D2.90]{quantale}. (A $\mathcal{V}$ matrix). \emph{Matrix multiplication} between $\mathcal{V}$ matrices
